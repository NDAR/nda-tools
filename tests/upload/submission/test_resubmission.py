...
def get_submission():
    ...
    submission = Submission(submission_status='Completed', dataset_title='best donuts data',
...
def test_check_replacement_authorized_not_authorized(monkeypatch, mock_config, get_submission_history):
    ...
    mock_submission_api.get_submission_history.return_value = [get_submission_history[0]]
...
def test_check_replacement_authorized_already_replaced(monkeypatch, mock_config, get_submission_history):
    mock_submission_api = MagicMock(spec=SubmissionApi)
    monkeypatch.setattr('NDATools.upload.submission.resubmission.SubmissionApi',
                        MagicMock(return_value=mock_submission_api))
    mock_submission_api.get_submission_history.return_value = get_submission_history
    ...
    If you need to make further edits to this submission, please reach out the the NDA help desk'''
        .format(submission_id, get_submission_history[0].created_by, get_submission_history[0].created_date))


def test_check_replacement_authorized_authorized(monkeypatch, mock_config, get_submission_history):
    mock_submission_api = MagicMock(spec=SubmissionApi)
    monkeypatch.setattr('NDATools.upload.submission.resubmission.SubmissionApi',
                        MagicMock(return_value=mock_submission_api))
    mock_submission_api.get_submission_history.return_value = [get_submission_history[1]]
...
def test_build_replacement_package_info_happy_path(monkeypatch, mock_config, mock_args, get_submission):
    ...
    mock_submission_api.get_submission_details.return_value = submission_details
...
def test_build_replacement_package_info_missing_data_discontinue(monkeypatch, mock_config, mock_args, get_submission):
...
def test_build_replacement_package_info_missing_data_continue(monkeypatch, mock_config, mock_args, get_submission):
    data_structure_details = DataStructureDetails(shortName=short_name, rows=4, validationUuids=[validation_uuid])
    submission_details = SubmissionDetails(validation_uuids=[validation_uuid], submissionId=submission_id,
                                           pendingChanges=[data_structure_details])
    ...
    replacement_package_info: ReplacementPackageInfo = build_replacement_package_info(validated_files=[validated_file], args=mock_args, config=mock_config)
