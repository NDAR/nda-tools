...
@pytest.fixture
def submission_api_mock(associated_file1, associated_file2, upload_creds1, upload_creds2):
...
def test_start_upload_happy_path(mock_get_cli, mock_s3_client, get_submission, datadir, submission_api_mock):
    ...
    verify_upload_context(upload_context=associated_file_uploader.uploader.upload_context,
                          ...
                          resuming_upload=False, search_folders=search_folders, num_of_files_not_found=0,
...
@patch('NDATools.upload.submission.associated_file.get_s3_client_with_config')
def test_start_upload_files_not_found_reenter(mock_get_cli, mock_input, mock_s3_client, get_submission, datadir,
                                              submission_api_mock, upload_creds1, upload_creds2):
...
def test_start_upload_files_not_found_exit(mock_get_cli, mock_input, mock_exit, mock_s3_client, get_submission,
    ...
    verify_upload_context(upload_context=associated_file_uploader.uploader.upload_context,
                          submission_id=get_submission.submission_id,
                          resuming_upload=False, search_folders=search_folders, num_of_files_not_found=1,
                          associated_file_count=2, uploaded_file_count=1)
...
@patch('NDATools.upload.submission.associated_file.get_s3_client_with_config')
def test_start_upload_s3_upload_error(mock_get_cli, mock_exit, mock_s3_client, get_submission, datadir,
                                      submission_api_mock):
    create_associated_files(datadir)
    search_folders = [datadir]
    ...
    verify_upload_context(upload_context=associated_file_uploader.uploader.upload_context,
                          submission_id=get_submission.submission_id,
                          resuming_upload=False, search_folders=search_folders, num_of_files_not_found=0,
                          associated_file_count=2, uploaded_file_count=0)
...
@patch('NDATools.upload.submission.associated_file.get_s3_client_with_config')
def test_start_upload_resume_upload(mock_get_cli, mock_s3_client, get_submission, datadir, submission_api_mock):
    create_associated_files(datadir)
    search_folders = [datadir]
    mock_get_cli.return_value = mock_s3_client
    ...
    verify_upload_context(upload_context=associated_file_uploader.uploader.upload_context,
                          submission_id=get_submission.submission_id,
                          resuming_upload=True, search_folders=search_folders, num_of_files_not_found=0,
                          associated_file_count=2, uploaded_file_count=2)
...
def test_start_upload_resume_xxl_upload_with_already_completed_files(mock_get_cli, mock_s3_client, get_submission,
                                                                     datadir, submission_api_mock, associated_file1,
                                                                     associated_file2):
    create_associated_files(datadir)
    search_folders = [datadir]
    mock_get_cli.return_value = mock_s3_client
    ...
    verify_upload_context(upload_context=associated_file_uploader.uploader.upload_context,
                          submission_id=get_submission.submission_id,
                          resuming_upload=True, search_folders=search_folders, num_of_files_not_found=0,
                          associated_file_count=1002, uploaded_file_count=1002)
...
@patch('NDATools.upload.submission.associated_file.get_s3_client_with_config')
def test_start_upload_resume_ignores_already_complete_status_errors(mock_get_cli, mock_s3_client, get_submission,
                                                                    datadir, submission_api_mock, associated_file1,
                                                                    upload_creds1):
    create_associated_file1(datadir)
    search_folders = [datadir]
...
@patch('NDATools.upload.submission.associated_file.get_s3_client_with_config')
def test_start_upload_rebuilds_incomplete_submission_db(mock_get_cli, mock_s3_client, get_submission, datadir,
                                                        submission_api_mock, associated_file1, associated_file2,
                                                        upload_creds1, upload_creds2):
    create_associated_files(datadir)
    search_folders = [datadir]
    mock_get_cli.return_value = mock_s3_client

    db_path = datadir / f'{get_submission.submission_id}.db'
    conn = sqlite3.connect(db_path)
    SqlUtils.create_table_from_model(conn, AssociatedFile, "associated_files")
    SqlUtils.bulk_insert(conn, "associated_files", [associated_file1])
    conn.close()

    submission_api_mock.get_files_by_page.side_effect = [[associated_file1, associated_file2], []]
    submission_api_mock.get_upload_credentials.side_effect = [[upload_creds1], [upload_creds2]]
    submission_api_mock.get_upload_progress.return_value = UploadProgress(associated_file_count=2, uploaded_file_count=0)
...
def verify_upload_context(upload_context, submission_id, resuming_upload, search_folders, num_of_files_not_found,
                          associated_file_count, uploaded_file_count):
    assert upload_context.submission.submission_id == submission_id
    assert upload_context.resuming_upload == resuming_upload
    assert upload_context.upload_progress.associated_file_count == associated_file_count
    assert upload_context.upload_progress.uploaded_file_count == uploaded_file_count
